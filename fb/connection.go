...
/*
#cgo LDFLAGS: -LC:/Users/Karim/Documents/Sources/FireBird -lfbclient
...
func (conn *Connection) check() error {
	if conn.db == C.FB_API_HANDLE(uintptr(0)) {
...
func (conn *Connection) Commit() (err error) {
	...
	if conn.transact != C.FB_API_HANDLE(uintptr(0)) {
...
func (conn *Connection) TransactionStarted() bool {
	return (conn.transact != C.FB_API_HANDLE(uintptr(0)))
}
